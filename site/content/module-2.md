...
[시작] 이 업무에 AI를 활용하려 한다
    │
    ▼
[질문 1] 입력할 데이터에 사내 기밀/개인정보가 포함되는가?
    │
    ├── YES ──▶ [사내 LLM 사용] ──▶ 4대 원칙 적용
    │
    └── NO
         │
         ▼
    [질문 2] 결과물이 외부 공개용인가, 내부 참고용인가?
         │
         ├── 외부 공개용 ──▶ [질문 3] 팩트 검증이 필요한가?
         │                       │
         │                       ├── YES ──▶ Perplexity(출처 확인) + Claude(문서화)
         ...
         [질문 3] 어떤 유형의 업무인가?
              │
              ├── 리서치/조사 ────▶ Perplexity + Gemini
              ├── 문서 작성 ──────▶ Claude or ChatGPT
              ├── 데이터 분석 ────▶ ChatGPT(Code Interpreter) or Gemini
              ├── 오피스 업무 ────▶ Copilot
              └── 멀티모달 분석 ──▶ Gemini
```
